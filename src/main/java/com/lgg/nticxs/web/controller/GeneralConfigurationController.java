...
public class GeneralConfigurationController {
    ...
    LotCardDAO lotcarddao = new LotCardDAO();
...
//    CardDAO carddao = new CardDAO();
   ...
   public String actionLotsGet(Model model,
	   ...
	   if(action.equals("add")) {
		   ...
		   model.addAttribute("listLot", listLot);
   ...
   public String actionCardLotsGet(Model model,
	...
	public String viewCardLotsGet(Model model,
		   ...
		   logger.logger("INFO", "SM-WEB", "General Configuration", "", "View Card Lots", "viewCardLotsGet()", "", "", 
				   "Name user: " + userdao.retrieveById(userId).getName(), 
				   "A screen opens where shows all card lots of a specific user");
   ...
   public String lotAddPost(Model model,
	   ...
	   if (action.compareTo("save") == 0) {
		   if (!(name.equals(""))) {
			   ...
			   Lot nameLote = lotdao.retrieveByNameLote(name);
   ...
   public String lotAddToUsersPost(Model model, HttpServletRequest request, 
		...
		if (action.compareTo("save") == 0) {
			...
			Lot lot = lotdao.retrieveByNameLote(nameLote);
...
//		}
    ...
    private void loadPage(Model model){
        ...
        List<Role> roles = addRoles();
        model.addAttribute("roleFound",roles);
        List<Lot> lotes = addLots();
        model.addAttribute("loteFound", lotes);
        List<LotCard> lotsCard = addCardLots();
        model.addAttribute("lotsCardsFound" , lotsCard);
    }
